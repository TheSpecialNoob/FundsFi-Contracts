...
library SafeBEP20 {
    ...
    function _callOptionalReturn(IBEP20 token, bytes memory data) private {
        ...
        if (returndata.length > 0) {
            ...
            // solhint-disable-next-line max-line-length
            require(abi.decode(returndata, (bool)), "SafeBEP20: HRC20 operation did not succeed");
        }
    }
}
...
contract FUNDSPresale is ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // The number of unclaimed FUNDS tokens the user has
    mapping(address => uint256) public FUNDSUnclaimed;
    // Last time user claimed FUNDS
    mapping(address => uint256) public lastFUNDSClaimed;
    ...
    // Total FUNDS sold
    uint256 public totalFUNDSSold = 0;

    // Price of presale FUNDS: 1 BuyToken
    uint256 private constant BuyTokenPerFUNDS = 300;
    ...
    uint256 public constant FUNDS_HARDCAP = 1500000*(1e6);
    ...
    function buy(uint256 _amount, address _buyer) public nonReentrant {
        ...
        uint256 tokens = _amount.div(BuyTokenPerFUNDS).mul(100);

        require(
            totalFUNDSSold + tokens <= FUNDS_HARDCAP,
            "FUNDS presale hardcap reached"
        );

        require(
            FUNDSUnclaimed[buyer] + tokens <= maxBuyPerUser,
            "Your amount exceeds the max buy number"
        );

        BuyToken.safeTransferFrom(buyer, address(this), _amount);

        FUNDSUnclaimed[buyer] = FUNDSUnclaimed[buyer].add(tokens);
        totalFUNDSSold = totalFUNDSSold.add(tokens);
    ...
    */
    ...
    function claim() external {
        ...
        require(
            FUNDSUnclaimed[msg.sender] > 0,
            "User should have unclaimed FUNDS tokens"
        );
        require(
            FUNDS.balanceOf(address(this)) >= FUNDSUnclaimed[msg.sender],
            "There are not enough FUNDS tokens to transfer."
        );

        if (lastFUNDSClaimed[msg.sender] == 0) {
            lastFUNDSClaimed[msg.sender] = firstHarvestTimestamp;
        }

        uint256 allowedPercentToClaim = block
        .timestamp
        .sub(lastFUNDSClaimed[msg.sender])
        .div(timePerPercent);

        lastFUNDSClaimed[msg.sender] = block.timestamp;
        ...
        uint256 FUNDSToClaim = FUNDSUnclaimed[msg.sender]
        .mul(allowedPercentToClaim)
        .div(100);
        FUNDSUnclaimed[msg.sender] = FUNDSUnclaimed[msg.sender].sub(FUNDSToClaim);

        FUNDSToClaim = FUNDSToClaim.mul(1e12);
        FUNDS.safeTransfer(msg.sender, FUNDSToClaim);
        emit TokenClaim(msg.sender, FUNDSToClaim);
    }
    ...
    function withdrawUnsoldFUNDS() external onlyOwner {
        uint256 amount = FUNDS.balanceOf(address(this)) - totalFUNDSSold.mul(1e12);
        FUNDS.safeTransfer(msg.sender, amount);
    }

    function emergencyWithdraw() external onlyOwner {
        FUNDS.safeTransfer(msg.sender, FUNDS.balanceOf(address(this)));
    }
    ...
    */
    ...
    function updateMaxBuyPerUser(uint256 _maxBuyPerUser) external onlyOwner {
        require(_maxBuyPerUser <= FUNDS_HARDCAP, "FUNDS::updateMaxBuyPerUser: maxBuyPerUser must not exceed the hardcap.");
